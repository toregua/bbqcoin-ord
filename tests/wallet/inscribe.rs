...
#[test]
fn inscribe_fails_if_bbqcoin_core_is_too_old() {
  ...
  CommandBuilder::new("wallet inscribe hello.txt")
    ...
    .expected_stderr(
      "error: Bbqcoin Core 1.14.6.0 or newer required, current version is 1.14.5.0\n",
    )
    .rpc_server(&rpc_server)
    .run();
}
...
fn inscribe_with_optional_satpoint_arg() {
  ...
  TestServer::spawn_with_args(&rpc_server, &["--index-sats"]).assert_response_regex(
    "/sat/100000000000000",
    format!(".*<a href=/bbqscription/{inscription}>.*"),
  );
