...
fn inscription_appears_on_reveal_transaction_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    format!("/tx/{reveal}"),
    format!(".*<h1>Transaction .*</h1>.*<a href=/bbqscription/{reveal}.*"),
  );
}
...
fn inscription_appears_on_output_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    ...
    format!(".*<h1>Output <span class=monospace>{reveal}:0</span></h1>.*<a href=/bbqscription/{inscription}.*"),
  );
}
...
fn inscription_page_after_send() {
  ...
  ord_server.assert_response_regex(
    format!("/bbqscription/{inscription}"),
    format!(
      r".*<h1>Bbqscription 0</h1>.*<dt>location</dt>\s*<dd class=monospace>{reveal}:0:0</dd>.*",
    ),
  );
  ...
  ord_server.assert_response_regex(
    format!("/bbqscription/{inscription}"),
    format!(
      r".*<h1>Bbqscription 0</h1>.*<dt>address</dt>\s*<dd class=monospace>bc1qcqgs2pps4u4yedfyl5pysdjjncs8et5utseepv</dd>.*<dt>location</dt>\s*<dd class=monospace>{send}:0:0</dd>.*",
    ),
  )
}
...
fn home_page_includes_latest_inscriptions() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    "/",
    format!(
      ".*<h2>Latest Bbqscriptions</h2>
<div class=thumbnails>
  <a href=/bbqscription/{inscription}><iframe .*></a>
</div>.*",
    ),
  );
}
...
fn inscriptions_page() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    ...
    format!(
      ".*<h1>Bbqscription</h1>
<div class=thumbnails>
  <a href=/bbqscription/{inscription}>.*</a>
</div>
.*",
    ),
  );
...
fn inscriptions_page_is_sorted() {
  ...
  for _ in 0..8 {
    let Inscribe { inscription, .. } = inscribe(&rpc_server);
    inscriptions.insert_str(0, &format!(".*<a href=/bbqscription/{inscription}>.*"));
  }

  TestServer::spawn_with_args(&rpc_server, &[])
    .assert_response_regex("/bbqscriptions", &inscriptions);
}
...
fn inscriptions_page_has_next_and_previous() {
  ...
  TestServer::spawn_with_args(&rpc_server, &[]).assert_response_regex(
    format!("/bbqscription/{b}"),
    format!(
      ".*<h1>Bbqscription 1</h1>.*
<div class=inscription>
<a class=prev href=/bbqscription/{a}>❮</a>
<iframe .* src=/preview/{b}></iframe>
<a class=next href=/bbqscription/{c}>❯</a>
</div>.*",
    ),
  );
}
