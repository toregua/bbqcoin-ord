...
git=toregua/ord-bbqcoin
crate=ord
url=https://github.com/toregua/ord-bbqcoin
...
if [ -z ${tag-} ]; then
  tag=$(curl --proto =https --tlsv1.2 -sSf https://api.github.com/repos/toregua/ord-bbqcoin/releases/latest |
    grep tag_name |
    cut -d'"' -f4
  )
fi
...
if [ -z ${target-} ]; then
  ...
  case $uname_target in
    ...
    *)
      ...
      err 'Please try building from source: https://github.com/toregua/ord-bbqcoin#building'
    ;;
  esac
fi
