...
use crate::bbq20::deploy::Deploy;
use crate::bbq20::errors::JSONError;
use crate::bbq20::mint::Mint;
use crate::bbq20::params::PROTOCOL_LITERAL;
use crate::bbq20::transfer::Transfer;
use crate::bbq20::OperationType;
...
pub(crate) fn deserialize_bbq20_operation(
  ...
  let raw_operation = match deserialize_bbq20(content_body) {
    Ok(op) => op,
    Err(e) => {
      return Err(e.into());
    }
  ...
  match action {
    ...
    Action::Transfer => match raw_operation {
      ...
      _ => Err(JSONError::NotBBQ20Json.into()),
    },
  }
}

fn deserialize_bbq20(s: &str) -> Result<RawOperation, JSONError> {
  ...
  if let Some(Value::String(p_value)) = identifier {
    if !p_value.eq_ignore_ascii_case(PROTOCOL_LITERAL) {
      return Err(JSONError::NotBBQ20Json);
    }
  } else {
    return Err(JSONError::NotBBQ20Json);
  }
