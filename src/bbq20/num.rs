...
use crate::bbq20::params::MAX_DECIMAL_WIDTH;

use super::BBQ20Error;

#[derive(PartialEq, PartialOrd, Debug, Clone)]
pub struct Num(BigDecimal);

impl Num {
  pub fn checked_add(&self, other: &Num) -> Result<Self, BBQ20Error> {
    Ok(Self(self.0.clone() + &other.0))
  }

  pub fn checked_sub(&self, other: &Num) -> Result<Self, BBQ20Error> {
    if self.0 < other.0 {
      return Err(BBQ20Error::Overflow {
  ...
  pub fn checked_mul(&self, other: &Num) -> Result<Self, BBQ20Error> {
    Ok(Self(self.0.clone() * &other.0))
  }

  pub fn checked_powu(&self, exp: u64) -> Result<Self, BBQ20Error> {
    match exp {
  ...
  pub fn checked_to_u8(&self) -> Result<u8, BBQ20Error> {
    if !self.0.is_integer() {
      return Err(BBQ20Error::InvalidInteger(self.clone().to_string()));
    }
    self.0.clone().to_u8().ok_or(BBQ20Error::Overflow {
  ...
  pub fn checked_to_u128(&self) -> Result<u128, BBQ20Error> {
    if !self.0.is_integer() {
      return Err(BBQ20Error::InvalidInteger(self.clone().to_string()));
    }
    self
      .0
      .to_bigint()
      .ok_or(BBQ20Error::InternalError(format!(
        "convert {} to bigint failed",
        self.0
      )))?
      .to_u128()
      .ok_or(BBQ20Error::Overflow {
...
impl FromStr for Num {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    if s.starts_with('.') || s.ends_with('.') || s.find(['e', 'E', '+', '-']).is_some() {
      return Err(BBQ20Error::InvalidNum(s.to_string()));
    }
    let num = BigDecimal::from_str(s).map_err(|_| BBQ20Error::InvalidNum(s.to_string()))?;
