...
use crate::bbq20::operation::{deserialize_bbq20_operation, Action, InscriptionOp, Operation};
use crate::bbq20::transfer::Transfer;
use crate::bbq20::TransferInfo;
...
impl Message {
  pub(crate) fn resolve<'a, 'db, 'tx>(
    bbq20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static InscriptionIdValue, &'static [u8]>,
    ...
    let bbq20_operation = match op.action {
      Action::New { inscription: _ } if sat_in_outputs => {
        match deserialize_bbq20_operation(
          new_inscriptions
          ...
          Ok(bbq20_operation) => bbq20_operation,
          _ => return Ok(None),
        }
      }
      // Transfered inscription operation.
      // Attempt to retrieve the `InscribeTransfer` Inscription information from the data store of BBQ20.
      Action::Transfer => {
        match get_inscribe_transfer_inscription(bbq20_inscribe_transfer, op.inscription_id) {
