use {
  self::{
    entry::{
      ...
      BuneEntryValue, BuneIdValue, OutPointMapValue, OutPointValue, SatPointValue, SatRange,
      TxidValue,
    },
    bunes::{Bune, BuneId},
    reorg::*,
...
use crate::bbq20::script_key::ScriptKey;
use crate::bbq20::{
...
pub(crate) use self::entry::BuneEntry;
...
define_table! { INSCRIPTION_ID_TO_BUNE, &InscriptionIdValue, u128 }
define_table! { INSCRIPTION_ID_TO_SATPOINT, &InscriptionIdValue, &SatPointValue }
define_table! { INSCRIPTION_NUMBER_TO_INSCRIPTION_ID, u64, &InscriptionIdValue }
define_table! { OUTPOINT_TO_BUNE_BALANCES, &OutPointValue, &[u8] }
...
define_table! { BUNE_ID_TO_BUNE_ENTRY, BuneIdValue, BuneEntryValue }
define_table! { BUNE_TO_BUNE_ID, u128, BuneIdValue }
...
define_table! { TRANSACTION_ID_TO_BUNE, &TxidValue, u128 }
define_table! { TRANSACTION_ID_TO_TRANSACTION, &TxidValue, &[u8] }
define_table! { WRITE_TRANSACTION_STARTING_BLOCK_COUNT_TO_TIMESTAMP, u32, u128 }
define_table! { BBQ20_BALANCES, &str, &[u8] }
define_table! { BBQ20_TOKEN, &str, &[u8] }
define_table! { BBQ20_INSCRIBE_TRANSFER, &InscriptionIdValue, &[u8] }
define_table! { BBQ20_TRANSFERABLELOG, &str, &[u8] }
define_multimap_table! { BBQ20_TOKEN_HOLDER, &str, &str}
...
impl Index {
  pub(crate) fn open(options: &Options) -> Result<Self> {
    ...
    let auth: Auth = if !cookie_file.exists() {
      ...
      log::info!(
        "Connecting to Bbqcoin Core RPC server at {rpc_url} using credentials from the url"
      );

      Auth::UserPass(username, password)
    } else {
      log::info!(
        "Connecting to Bbqcoin Core RPC server at {rpc_url} using credentials from `{}`",
        cookie_file.display()
      );
    ...
    let database = match unsafe { Database::builder().open(&path) } {
      Ok(database) => {
        {
          ...
          index_bunes = statistics
            .get(&Statistic::IndexBunes.key())?
            .unwrap()
            .value()
            != 0;
          ...
          index_transactions = statistics
            ...
            .value()
            != 0;
          index_bbq20 = statistics
            .get(&Statistic::IndexBbq20.key())?
            .unwrap()
            .value()
            != 0;
        }
        ...
        if error.kind() == io::ErrorKind::NotFound =>
      {
        ...
        tx.open_table(INSCRIPTION_ID_TO_BUNE)?;
...

        {
          ...
          index_bbq20 = options.index_bunes();
          index_bunes = options.index_bunes();
          index_sats = options.index_sats;
          index_transactions = options.index_transactions;

          statistics.insert(&Statistic::IndexBbq20.key(), &u64::from(index_bbq20))?;

          statistics.insert(&Statistic::IndexBunes.key(), &u64::from(index_bunes))?;
    ...
    Ok(Self {
      ...
      first_bune_height: options.first_bune_height(),
  ...
  pub(crate) fn get_unspent_outputs(&self, _wallet: Wallet) -> Result<BTreeMap<OutPoint, Amount>> {
    ...
    for outpoint in utxos.keys() {
      if outpoint_to_value.get(&outpoint.store())?.is_none() {
        return Err(anyhow!(
          "output in Bbqcoin Core wallet but not in ord index: {outpoint}"
        ));
      }
    }
  ...
  pub(crate) fn has_bune_index(&self) -> bool {
    self.index_bunes
  }
  ...
  pub(crate) fn get_bune_by_id(&self, id: BuneId) -> Result<Option<Bune>> {
    Ok(
      self
        .database
        .begin_read()?
        .open_table(BUNE_ID_TO_BUNE_ENTRY)?
        .get(&id.store())?
        .map(|entry| BuneEntry::load(entry.value()).bune),
    )
  }

  pub(crate) fn bune(&self, bune: Bune) -> Result<Option<(BuneId, BuneEntry)>> {
    let rtx = self.database.begin_read()?;

    let entry = match rtx.open_table(BUNE_TO_BUNE_ID)?.get(bune.0)? {
      Some(id) => rtx
        .open_table(BUNE_ID_TO_BUNE_ENTRY)?
        .get(id.value())?
        .map(|entry| (BuneId::load(id.value()), BuneEntry::load(entry.value()))),
      None => None,
    };
  ...
  pub(crate) fn bunes(&self) -> Result<Vec<(BuneId, BuneEntry)>> {
    ...
    {
      let (id, entry) = result?;
      entries.push((BuneId::load(id.value()), BuneEntry::load(entry.value())));
    }

    Ok(entries)
  }

  pub(crate) fn get_bune_balance(&self, outpoint: OutPoint, id: BuneId) -> Result<u128> {
    if self.block_count()? >= self.first_bune_height && self.index_bunes {
      let rtx = self.database.begin_read()?;

      let outpoint_to_balances = rtx.open_table(OUTPOINT_TO_BUNE_BALANCES)?;
      ...
      while i < balances_buffer.len() {
        let (balance_id, length) = bunes::varint::decode(&balances_buffer[i..]);
        i += length;
        let (amount, length) = bunes::varint::decode(&balances_buffer[i..]);
        i += length;

        if BuneId::try_from(balance_id).unwrap() == id {
          return Ok(amount);
        }
      }
    }
  ...
  pub(crate) fn get_bune_balances_for_outpoint(
    &self,
    outpoint: OutPoint,
  ) -> Result<Vec<(SpacedBune, Pile)>> {
    if self.block_count()? >= self.first_bune_height && self.index_bunes {
      let rtx = &self.database.begin_read()?;

      let outpoint_to_balances = rtx.open_table(OUTPOINT_TO_BUNE_BALANCES)?;

      let id_to_bune_entries = rtx.open_table(BUNE_ID_TO_BUNE_ENTRY)?;
      ...
      let mut i = 0;
      while i < balances_buffer.len() {
        let (id, length) = bunes::varint::decode(&balances_buffer[i..]);
        i += length;
        let (amount, length) = bunes::varint::decode(&balances_buffer[i..]);
        i += length;
        ...
        let entry = BuneEntry::load(id_to_bune_entries.get(id.store())?.unwrap().value());
  ...
  pub(crate) fn get_bune_balance_map(
    &self,
  ) -> Result<BTreeMap<SpacedBune, BTreeMap<OutPoint, u128>>> {
    let outpoint_balances = self.get_bune_balances()?;

    let rtx = self.database.begin_read()?;

    let bune_id_to_bune_entry = rtx.open_table(BUNE_ID_TO_BUNE_ENTRY)?;

    let mut bune_balances: BTreeMap<SpacedBune, BTreeMap<OutPoint, u128>> = BTreeMap::new();

    for (outpoint, balances) in outpoint_balances {
      for (bune_id, amount) in balances {
        let spaced_bune = BuneEntry::load(
          bune_id_to_bune_entry
            .get(&bune_id.store())?
            .unwrap()
            .value(),
        )
  ...
  pub(crate) fn get_bune_balances(&self) -> Result<Vec<(OutPoint, Vec<(BuneId, u128)>)>> {
    ...
    if self.block_count()? >= self.first_bune_height && self.index_bunes {
      for entry in self
        .database
        .begin_read()?
        .open_table(OUTPOINT_TO_BUNE_BALANCES)?
        .iter()?
      {
        ...
        while i < balances_buffer.len() {
          let (id, length) = bunes::varint::decode(&balances_buffer[i..]);
          i += length;
          let (balance, length) = bunes::varint::decode(&balances_buffer[i..]);
          i += length;
          balances.push((BuneId::try_from(id)?, balance));
        }
  ...
  pub(crate) fn get_bbq20_balances(&self, script_key: &ScriptKey) -> Result<Vec<Balance>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let bbq20_token_balance = rtx.open_table(BBQ20_BALANCES)?;
  ...
  pub(crate) fn get_bbq20_balance(
    &self,
  ...
  pub(crate) fn get_bbq20_token_info(&self, tick: &Tick) -> Result<Option<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let bbq20_token_info = rtx.open_table(BBQ20_TOKEN)?;
      return Ok(
        bbq20_token_info
  ...
  pub(crate) fn get_bbq20_tokens_info(&self) -> Result<Vec<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;
  ...
  pub(crate) fn get_bbq20_token_holder(&self, tick: &Tick) -> Result<Vec<ScriptKey>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let bbq20_token_holder = rtx.open_multimap_table(BBQ20_TOKEN_HOLDER)?;
  ...
  pub(crate) fn get_bbq20_transferable_by_range(
    &self,
    script: &ScriptKey,
  ) -> Result<Vec<TransferableLog>, redb::Error> {
    let rtx = self.database.begin_read()?;
    let bbq20_transferable_log = rtx.open_table(BBQ20_TRANSFERABLELOG)?;
    let result = Ok(
      bbq20_transferable_log
  ...
  pub(crate) fn get_bbq20_transferable_by_tick(
    &self,
    script: &ScriptKey,
    tick: &Tick,
  ) -> Result<Vec<TransferableLog>, redb::Error> {
  ...
  pub(crate) fn get_bbq20_transferable_by_id(
    &self,
    script_key: &ScriptKey,
    ...
    if self.block_count().unwrap() >= self.first_inscription_height {
      ...
      let transferable_log_vec: Vec<TransferableLog> = bbq20_transferable_log
  ...
  pub(crate) fn get_etching(&self, txid: Txid) -> Result<Option<SpacedBune>> {
    if self.block_count().unwrap() >= self.first_bune_height {
      let rtx = self.database.begin_read()?;

      let transaction_id_to_bune = rtx.open_table(TRANSACTION_ID_TO_BUNE)?;
      let Some(bune) = transaction_id_to_bune.get(&txid.store())? else {
        return Ok(None);
      };

      let bune_to_bune_id = rtx.open_table(BUNE_TO_BUNE_ID)?;
      let id = bune_to_bune_id.get(bune.value())?.unwrap();

      let bune_id_to_bune_entry = rtx.open_table(BUNE_ID_TO_BUNE_ENTRY)?;
      let entry = bune_id_to_bune_entry.get(&id.value())?.unwrap();

      Ok(Some(BuneEntry::load(entry.value()).spaced_bune()))
    } else {
      Ok(None)
    }
  }
  ...
  pub(crate) fn get_bune_by_inscription_id(
    &self,
    inscription_id: InscriptionId,
  ) -> Result<Option<SpacedBune>> {
    let rtx = self.database.begin_read()?;
    let Some(bune) = rtx
      .open_table(INSCRIPTION_ID_TO_BUNE)?
      .get(&inscription_id.store())?
      .map(|entry| Bune(entry.value()))
    else {
      return Ok(None);
    };
    let bune_to_bune_id = rtx.open_table(BUNE_TO_BUNE_ID)?;
    let id = bune_to_bune_id.get(bune.0)?.unwrap();
...
mod tests {
  ...
  #[test]
  #[ignore]
  fn missing_inputs_are_fetched_from_bbqcoin_core() {
    for args in [
    ...
    for context in Context::configurations() {
      ...
      assert_regex_match!(
        context
          ...
          .unwrap_err()
          .to_string(),
        r"output in Bbqcoin Core wallet but not in ord index: [[:xdigit:]]{64}:\d+"
      );
    }
  }
