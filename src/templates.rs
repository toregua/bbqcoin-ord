...
pub(crate) use {
  bbq20::{BBQ20Balance, BBQ20Output, BBQ20UtxoOutput, Operation, BBQ20},
  block::BlockHashAndConfirmations,
  block::BlockHtml,
  block::BlockJson,
  bune::{
    BuneAddressJson, BuneBalance, BuneEntryJson, BuneHtml, BuneJson, BuneOutput, BuneOutputJson,
  },
  bune_balances::BuneBalancesHtml,
  bunes::BunesHtml,
  home::HomeHtml,
  iframe::Iframe,
  input::InputHtml,
  inscription::{BbqscriptionJson, InscriptionByAddressJson, InscriptionHtml, InscriptionJson},
...
<html lang=en>
  ...
  <header>
    <nav>
      <a href=/>Bbqnals<sup>alpha</sup></a>
      .*
...
"
  ...
  fn page_mainnet() {
    assert_regex_match!(
      ...
      r".*<nav>\s*<a href=/>Bbqnals<sup>alpha</sup></a>.*"
    );
  }
  ...
  fn page_no_sat_index() {
    assert_regex_match!(
      ...
      r".*<nav>\s*<a href=/>Bbqnals<sup>alpha</sup></a>.*\s*<form action=/search.*",
    );
  }
  ...
  fn page_signet() {
    assert_regex_match!(
      ...
      r".*<nav>\s*<a href=/>Bbqnals<sup>signet</sup></a>.*"
    );
  }
}
