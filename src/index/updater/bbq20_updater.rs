...
use crate::bbq20::errors::Error::LedgerError;
use crate::bbq20::operation::{InscriptionOp, Operation};
use crate::bbq20::params::{BIGDECIMAL_TEN, MAX_DECIMAL_WIDTH};
...
use crate::bbq20::{
  ...
  InscribeTransferEvent, BBQ20Error, Message, Mint, MintEvent, Num, Tick, TokenInfo, Transfer,
...
pub(super) struct Bbq20Updater<'a, 'db, 'tx> {
  bbq20_token_info: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
  bbq20_token_holder: &'a mut MultimapTable<'db, 'tx, &'static str, &'static str>,
  bbq20_token_balance: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
  bbq20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static [u8; 36], &'static [u8]>,
  bbq20_transferable_log: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
...
impl<'a, 'db, 'tx> Bbq20Updater<'a, 'db, 'tx> {
  pub(super) fn new(
    bbq20_token_info: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
    bbq20_token_holder: &'a mut MultimapTable<'db, 'tx, &'static str, &'static str>,
    bbq20_token_balance: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
  ...
  pub(crate) fn index_block(
    ...
    log::info!(
      "BBQ20 Updater indexed block {} with {} messages in {} ms",
      context.blockheight,
      messages_size,
  ...
  pub fn resolve_message(
    ...
    for input in &tx.input {
      ...
      while let Some(operation) = operation_iter.peek() {
        ...
        // Parse BBQ20 message through inscription operation.
        if let Some(msg) = Message::resolve(
          &mut self.bbq20_inscribe_transfer,
          &new_inscriptions,
          operation,
        )? {
          messages.push(msg);
  ...
  fn process_deploy(
    ...
    deploy: Deploy,
  ) -> Result<Event, errors::Error<BBQ20Error>> {
    // ignore inscribe inscription to coinbase.
    let to_script_key = msg.to.clone().ok_or(BBQ20Error::InscribeToCoinbase)?;

    let tick = deploy.tick.parse::<Tick>()?;

    if let Some(stored_tick_info) = Self::get_token_info(self, &tick).map_err(|e| LedgerError(e))? {
      return Err(errors::Error::BBQ20Error(BBQ20Error::DuplicateTick(
    ...
    if dec > MAX_DECIMAL_WIDTH {
      return Err(errors::Error::BBQ20Error(BBQ20Error::DecimalsTooLarge(dec)));
    }
    let base = BIGDECIMAL_TEN.checked_powu(u64::from(dec))?;

    let supply = Num::from_str(&deploy.max_supply)?;

    if supply.sign() == Sign::NoSign || supply > bbq20::params::MAXIMUM_SUPPLY.to_owned() {
      return Err(errors::Error::BBQ20Error(BBQ20Error::InvalidSupply(
        supply.to_string(),
      )));
    }
    ...
    if limit.sign() == Sign::NoSign || limit > bbq20::params::MAXIMUM_SUPPLY.to_owned() {
      return Err(errors::Error::BBQ20Error(BBQ20Error::MintLimitOutOfRange(
  ...
  fn process_mint(
    ...
    let token_info = Self::get_token_info(self, &tick)
      ...
      .ok_or(BBQ20Error::TickNotFound(tick.to_string()))?;
    ...
    if amt.scale() > i64::from(token_info.decimal) {
      return Err(errors::Error::BBQ20Error(BBQ20Error::AmountOverflow(
        amt.to_string(),
      )));
    }
    ...
    if amt.sign() == Sign::NoSign {
      return Err(errors::Error::BBQ20Error(BBQ20Error::InvalidZeroAmount));
    }
    if amt > Into::<Num>::into(token_info.limit_per_mint) {
      return Err(errors::Error::BBQ20Error(BBQ20Error::AmountExceedLimit(
        amt.to_string(),
      )));
    }
    ...
    if minted >= supply {
      return Err(errors::Error::BBQ20Error(BBQ20Error::TickMinted(
  ...
  fn process_inscribe_transfer(
    ...
    if available < amt {
      return Err(errors::Error::BBQ20Error(BBQ20Error::InsufficientBalance(
        available.to_string(),
        amt.to_string(),
      )));
    }
  ...
  fn process_transfer(
    ...
    let mut transferable = Self::get_transferable_by_id(self, &msg.from, &msg.inscription_id)
      .map_err(|e| LedgerError(e))?
      .ok_or(BBQ20Error::TransferableNotFound(msg.inscription_id))?;
    let amt = Into::<Num>::into(transferable.amount);

    if transferable.owner != msg.from {
      return Err(errors::Error::BBQ20Error(
        BBQ20Error::TransferableOwnerNotMatch(msg.inscription_id),
      ));
    }
  ...
  fn insert_transferable(
    ...
    self.bbq20_transferable_log.insert(
  ...
  fn insert_inscribe_transfer_inscription(
    ...
    self.bbq20_inscribe_transfer.insert(
  ...
  fn update_token_balance(
    ...
    self.bbq20_token_balance.insert(
  ...
  fn remove_token_holder(
    ...
    self.bbq20_token_holder.remove(
      tick.to_lowercase().hex().as_str(),
  ...
  fn insert_token_holder(&mut self, script_key: &ScriptKey, tick: Tick) -> Result<(), redb::Error> {
    self.bbq20_token_holder.insert(
      tick.to_lowercase().hex().as_str(),
      script_key.to_string().as_str(),
    )?;
    Ok(())
