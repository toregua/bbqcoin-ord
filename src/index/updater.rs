...
use {
  self::{inscription_updater::InscriptionUpdater, bune_updater::BuneUpdater},
...
use crate::bbq20::BlockContext;
use crate::index::updater::bbq20_updater::Bbq20Updater;
use crate::sat::Sat;
...
impl<'index> Updater<'_> {
  ...
  fn index_block(
    ...
    let mut bbq20_token_info = wtx.open_table(BBQ20_TOKEN)?;
    let mut bbq20_token_holder = wtx.open_multimap_table(BBQ20_TOKEN_HOLDER)?;
    let mut bbq20_token_balance = wtx.open_table(BBQ20_BALANCES)?;
    let mut bbq20_inscribe_transfer = wtx.open_table(BBQ20_INSCRIBE_TRANSFER)?;
    let mut bbq20_transferable_log = wtx.open_table(BBQ20_TRANSFERABLELOG)?;
...

    {
      ...
      if index.index_bbq20 && self.height >= index.first_inscription_height {
        let operations = inscription_updater.operations.clone();

        // Create a protocol manager to index the block of bbq20 data.
        Bbq20Updater::new(
          &mut bbq20_token_info,
          &mut bbq20_token_holder,
          &mut bbq20_token_balance,
          &mut bbq20_inscribe_transfer,
          &mut bbq20_transferable_log,
    ...
    if index.index_bunes && self.height >= self.index.first_bune_height {
      let mut outpoint_to_bune_balances = wtx.open_table(OUTPOINT_TO_BUNE_BALANCES)?;
      let mut bune_id_to_bune_entry = wtx.open_table(BUNE_ID_TO_BUNE_ENTRY)?;
      let mut bune_to_bune_id = wtx.open_table(BUNE_TO_BUNE_ID)?;
      let mut inscription_id_to_bune = wtx.open_table(INSCRIPTION_ID_TO_BUNE)?;
      let mut bune_updater = BuneUpdater::new(
        self.height,
        &mut outpoint_to_bune_balances,
        &mut bune_id_to_bune_entry,
        &inscription_id_to_inscription_entry,
        &mut inscription_id_to_bune,
        &mut bune_to_bune_id,
        &mut statistic_to_count,
        block.header.time,
        Bune::minimum_at_height(Chain::Mainnet, Height(self.height)),
      )?;
      for (i, (tx, txid)) in block.txdata.iter().enumerate() {
        bune_updater.index_bunes(i, tx, *txid)?;
      }
    }
