...
impl Context {
  ...
  #[track_caller]
  pub(crate) fn assert_bunes(
    &self,
    mut bunes: impl AsMut<[(BuneId, BuneEntry)]>,
    mut balances: impl AsMut<[(OutPoint, Vec<(BuneId, u128)>)]>,
  ) {
    let bunes = bunes.as_mut();
    bunes.sort_by_key(|(id, _)| *id);
    ...
    assert_eq!(bunes, self.index.bunes().unwrap());

    assert_eq!(balances, self.index.get_bune_balances());

    let mut outstanding: HashMap<BuneId, u128> = HashMap::new();
