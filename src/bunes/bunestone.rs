...
impl Bunestone {
  ...
  fn decipher(transaction: &Transaction) -> Result<Option<Self>, script::Error> {
    let Some(payload) = Bunestone::payload(transaction)? else {
      return Ok(None);
    };

    let integers = Bunestone::integers(&payload);
    ...
    let bune = Tag::Bune.take(&mut fields).map(Bune);
  ...
  pub(crate) fn encipher(&self) -> Script {
    ...
    if let Some(etching) = self.etching {
      ...
      if let Some(bune) = etching.bune {
        Tag::Bune.encode(bune.0, &mut payload);
      }
...
mod tests {
  ...
  fn from_transaction_returns_none_if_decipher_returns_error() {
    assert_eq!(
      Bunestone::from_transaction(&Transaction {
        input: Vec::new(),
        output: vec![TxOut {
  ...
  fn deciphering_transaction_with_no_outputs_returns_none() {
    assert_eq!(
      Bunestone::decipher(&Transaction {
        input: Vec::new(),
        output: Vec::new(),
        lock_time: PackedLockTime::ZERO,
        version: 0,
  ...
  #[test]
  fn deciphering_valid_bunestone_with_invalid_script_returns_script_error() {
    let result = Bunestone::decipher(&Transaction {
      input: Vec::new(),
      output: vec![TxOut {
        script_pubkey: Script::from_bytes(vec![opcodes::all::OP_PUSHBYTES_4.to_u8()]),
        value: 0,
  ...
  #[test]
  fn deciphering_valid_bunestone_with_invalid_script_postfix_returns_script_error() {
  ...
  #[test]
  fn deciphering_bunestone_with_invalid_varint_returns_varint_error() {
    let result = Bunestone::decipher(&Transaction {
      input: Vec::new(),
      output: vec![TxOut {
        script_pubkey: script::Builder::new()
  ...
  #[test]
  fn non_push_opcodes_in_bunestone_are_ignored() {
    assert_eq!(
      Bunestone::decipher(&Transaction {
        input: Vec::new(),
        output: vec![TxOut {
  ...
  #[test]
  fn deciphering_empty_bunestone_is_successful() {
    assert_eq!(
      Bunestone::decipher(&Transaction {
        input: Vec::new(),
        output: vec![TxOut {
  ...
  #[test]
  fn error_in_input_aborts_search_for_bunestone() {
  ...
  #[test]
  fn deciphering_non_empty_bunestone_is_successful() {
    let payload = payload(&[0, 1, 2, 3]);
  ...
  #[test]
  fn bunestone_may_contain_multiple_edicts() {
  ...
  #[test]
  fn bunestone_may_be_in_second_output() {
    let payload = payload(&[0, 1, 2, 3]);
  ...
  #[test]
  fn bunestone_may_be_after_non_matching_op_return() {
    let payload = payload(&[0, 1, 2, 3]);
