...
impl Bune {
  ...
  pub(crate) fn minimum_at_height(chain: Chain, height: Height) -> Self {
    ...
    let start = chain.first_bune_height();

    let end = start + SUBSIDY_HALVING_INTERVAL_10X;

    if offset < start {
      return Bune(Self::STEPS[12]);
    }
    ...
    Bune(start - ((start - end) * remainder / u128::from(INTERVAL)))
  }
  ...
  pub(crate) fn reserved(n: u128) -> Self {
    Bune(RESERVED.checked_add(n).unwrap())
  }
}

impl Serialize for Bune {
...
impl<'de> Deserialize<'de> for Bune {
  ...
  fn from_str(s: &str) -> crate::Result<Self> {
    ...
    for (i, c) in s.chars().enumerate() {
      ...
      match c {
        ...
        _ => bail!("invalid character in bune name: {c}"),
      }
    }
    Ok(Bune(x))
  }
...
mod tests {
  ...
  fn round_trip() {
    fn case(n: u128, s: &str) {
      assert_eq!(Bune(n).to_string(), s);
      assert_eq!(s.parse::<Bune>().unwrap(), Bune(n));
    }
  ...
  fn from_str_out_of_range() {
    "BCGDENLQRQWDSLRUGSNLBTMFIJAW".parse::<Bune>().unwrap_err();
  }
  ...
  fn mainnet_minimum_at_height() {
    ...
    fn case(height: u32, minimum: &str) {
      assert_eq!(
        Bune::minimum_at_height(Chain::Mainnet, Height(height)).to_string(),
        minimum,
      );
    }
  ...
  fn minimum_at_height() {
    ...
    fn case(chain: Chain, height: u32, minimum: &str) {
      assert_eq!(
        Bune::minimum_at_height(chain, Height(height as u64)).to_string(),
        minimum,
      );
    }
  ...
  fn serde() {
    let bune = Bune(0);
    let json = "\"A\"";
    assert_eq!(serde_json::to_string(&bune).unwrap(), json);
    assert_eq!(serde_json::from_str::<Bune>(json).unwrap(), bune);
  }
  ...
  fn reserved() {
    assert_eq!(
      RESERVED,
      "AAAAAAAAAAAAAAAAAAAAAAAAAAA".parse::<Bune>().unwrap().0,
    );

    assert_eq!(Bune::reserved(0), Bune(RESERVED));
    assert_eq!(Bune::reserved(1), Bune(RESERVED + 1));
  }

  #[test]
  fn is_reserved() {
    #[track_caller]
    fn case(bune: &str, reserved: bool) {
      assert_eq!(bune.parse::<Bune>().unwrap().is_reserved(), reserved);
    }
  ...
  fn steps() {
    for i in 0.. {
      match "A".repeat(i + 1).parse::<Bune>() {
        Ok(bune) => assert_eq!(Bune(Bune::STEPS[i]), bune),
        Err(_) => {
          assert_eq!(Bune::STEPS.len(), i);
          break;
        }
      }
    }
