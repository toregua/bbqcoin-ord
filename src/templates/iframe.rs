...
impl Display for Iframe {
  fn fmt(&self, f: &mut Formatter) -> fmt::Result {
    if self.thumbnail {
      write!(f, "<a href=/bbqscription/{}>", self.inscription_id)?;
    }
...
mod tests {
  ...
  fn thumbnail() {
    assert_regex_match!(
      ...
      .0.to_string(),
      "<a href=/bbqscription/1{64}i1><iframe sandbox=allow-scripts scrolling=no loading=lazy src=/preview/1{64}i1></iframe></a>",
    );
  }
