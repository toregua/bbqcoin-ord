...
impl Send {
  pub(crate) fn run(self, options: Options) -> SubcommandResult {
    ...
    let satpoint = match self.outgoing {
      ...
      Outgoing::Bune { decimal, bune } => {
        let transaction = Self::send_bunes(
          address,
          &client,
          decimal,
          self.fee_rate,
  ...
  fn send_bunes(
    ...
    spaced_bune: SpacedBune,
    dunic_outputs: BTreeSet<OutPoint>,
    unspent_outputs: BTreeMap<OutPoint, Amount>,
  ) -> Result<Txid> {
    ensure!(
      index.has_bune_index(),
      "sending bunes with `ord send` requires index created with `--index-bunes` flag",
    );

    let (id, entry) = index
      .bune(spaced_bune.bune)?
      .with_context(|| format!("bune `{}` has not been etched", spaced_bune.bune))?;
    ...
    for output in dunic_outputs {
      ...
      let balance = index.get_bune_balance(output, id)?;
    ...
    let unfunded_transaction = Transaction {
      ...
      output: vec![
        TxOut {
          script_pubkey: bunestone.encipher(),
          value: 0,
        },
        TxOut {
