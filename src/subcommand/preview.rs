...
impl Preview {
  pub(crate) fn run(self) -> SubcommandResult {
    ...
    let bbqcoin_data_dir = tmpdir.path().join("bbqcoin");

    fs::create_dir(&bbqcoin_data_dir)?;

    let _bbqcoind = KillOnDrop(
      Command::new("bbqcoind")
        .arg({
          let mut arg = OsString::from("-datadir=");
          arg.push(&bbqcoin_data_dir);
          arg
        })
        .arg("-regtest")
        .arg("-txindex")
        ...
        .spawn()
        .context("failed to spawn `bbqcoind`")?,
    );

    let options = Options {
      chain_argument: Chain::Regtest,
      bbqcoin_data_dir: Some(bbqcoin_data_dir),
    ...
    for attempt in 0.. {
      if options.bbqcoin_rpc_client().is_ok() {
        break;
      }

      if attempt == 100 {
        panic!("Bbqcoin Core RPC did not respond");
      }
    ...
    let rpc_client = options.bbqcoin_rpc_client_for_wallet_command(false)?;
